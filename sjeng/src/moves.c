...
bool_c check_legal (move_s moves[], int m, int incheck) {
	...
	/* determines if a move made was legal.  Checks to see if the player who
     ...
     was illegal, returns FALSE_C, otherwise, returns TRUE_C. */
	...
	if (Variant == Suicide) return TRUE_C;
	...
	if (castled) 
	{
		...
		if (castled == wck) {
			if (is_attacked (30, 0)) return FALSE_C;
			if (is_attacked (31, 0)) return FALSE_C;
			if (is_attacked (32, 0)) return FALSE_C;
			return TRUE_C;
		}
		/* white queenside castling: */
		if (castled == wcq) {
			if (is_attacked (30, 0)) return FALSE_C;
			if (is_attacked (29, 0)) return FALSE_C;
			if (is_attacked (28, 0)) return FALSE_C;
			return TRUE_C;
		}
		/* black kingside castling: */
		if (castled == bck) {
			if (is_attacked (114, 1)) return FALSE_C;
			if (is_attacked (115, 1)) return FALSE_C;
			if (is_attacked (116, 1)) return FALSE_C;
			return TRUE_C;
		}
		/* black queenside castling: */
		if (castled == bcq) {
			if (is_attacked (114, 1)) return FALSE_C;
			if (is_attacked (113, 1)) return FALSE_C;
			if (is_attacked (112, 1)) return FALSE_C;
			return TRUE_C;
		}
	}
	...
	else if (white_to_move&1) 
    {
		if (!incheck)
		{
			if (moves[m].from == 0) return TRUE_C;
			...
			if (board[target] != bking)
			{
				if (file(from) == file(bking_loc))
				{
					if (bking_loc > from)
					{
						...
						if (board[l] == wrook || board[l] == wqueen) return FALSE_C;
					}
					else
					{
						for (l = bking_loc+12; board[l] == npiece; l+=12);
				...
				else if (diagl(from) == diagl(bking_loc))
				{
					if (bking_loc > from)
					{
						...
						if (board[l] == wbishop || board[l] == wqueen) return FALSE_C;
					}
					else
					{
						for (l = bking_loc+13; board[l] == npiece; l+=13);
		...
		if (is_attacked (bking_loc, 1)) return FALSE_C;
		else return TRUE_C;
    }
	...
	else 
    {
			...
			if (board[target] != wking)
			{
				if (file(from) == file(wking_loc))
				{
					if (wking_loc > from)
					{
						...
						if (board[l] == brook || board[l] == bqueen) return FALSE_C;
					}
					else
					{
						for (l = wking_loc+12; board[l] == npiece; l+=12);
				...
				else if (diagl(from) == diagl(wking_loc))
				{
					if (wking_loc > from)
					{
						...
						if (board[l] == bbishop || board[l] == bqueen) return FALSE_C;
					}
					else
					{
						for (l = wking_loc+13; board[l] == npiece; l+=13);
		...
		if (is_attacked (wking_loc, 0)) return FALSE_C;
		else return TRUE_C;
	}
...
restart:
			...
			switch (board[from]) {
				case (wpawn):
					...
					if (board[from+12] == npiece) {
						/* only promotions when captures == TRUE_C */
						if (rank (from) == 7 && ((Variant != Suicide) && (Variant != Losers))) {
							push_pawn (from+12, FALSE_C);
						}
						else if (!captures) {
							push_pawn (from+12, FALSE_C);
				...
				case (wking):
					...
					push_king (from+13);
					/* castling moves: */
					if (from == 30 && !moved[30] && !captures && (Variant != Suicide || Giveaway == TRUE_C)) {
						/* kingside: */
		...
		for (a = 1, j = 1;
			...
			switch (board[from]) {
				...
				case (bking):
					...
					push_king (from+13);
					/* castling moves: */
					if (from == 114 && !moved[114] && !captures && (Variant != Suicide || Giveaway == TRUE_C)) {
						/* kingside: */
	...
	if ((Variant == Suicide) && fcaptures == TRUE_C && captures == FALSE_C)
    {
		captures = TRUE_C;
		numb_moves = 0;
		goto restart;
	...
	if (Variant == Suicide) kingcap = FALSE_C;
	
	
}


bool_c in_check (void) {
	
	/* return true if the side to move is in check: */
	
	if (Variant == Suicide) return FALSE_C;
...
bool_c f_in_check(move_s moves[], int m)
{
	int target = moves[m].target;
	int from = moves[m].from;
	int l;
	...
	if (white_to_move == 1)
	{
		...
		{
			case bpawn:
				if (board[target-11] == wking || board[target-13] == wking) return TRUE_C;
				break;
			case bbishop:
			...
			case bknight:
				for (l = 0; l < 8; l++) 
					if ((wking_loc + knight_o[l]) == target) return TRUE_C;
				break;
			case bqueen:
				if (file(target) == file(wking_loc))
				{
		...
		if (rank(from) == rank(wking_loc))
		{
			...
			{
				for (l = wking_loc-1; board[l] == npiece; l--);    
				if (board[l] == brook || board[l] == bqueen) return TRUE_C;
			}
			else
			{
				for (l = wking_loc+1; board[l] == npiece; l++);
		...
		else if (diagl(from) == diagl(wking_loc))
		{
			...
			{
				for (l = wking_loc-13; board[l] == npiece; l-=13);    
				if (board[l] == bbishop || board[l] == bqueen) return TRUE_C;
			}
			else
			{
				for (l = wking_loc+13; board[l] == npiece; l+=13);
	...
	else
	{
		...
		{
			case wpawn:
				if (board[target+11] == bking || board[target+13] == bking) return TRUE_C;
				break;
			case wbishop:
			...
			case wknight:
				for (l = 0; l < 8; l++) 
					if ((bking_loc + knight_o[l]) == target) return TRUE_C;
				break;
			case wqueen:
				if (file(target) == file(bking_loc))
				{
		...
		if (rank(from) == rank(bking_loc))
		{
			...
			{
				for (l = bking_loc-1; board[l] == npiece; l--);    
				if (board[l] == wrook || board[l] == wqueen) return TRUE_C;
			}
			else
			{
				for (l = bking_loc+1; board[l] == npiece; l++);
		...
		else if (diagl(from) == diagl(bking_loc))
		{
			...
			{
				for (l = bking_loc-13; board[l] == npiece; l-=13);    
				if (board[l] == wbishop || board[l] == wqueen) return TRUE_C;
			}
			else
			{
				for (l = bking_loc+13; board[l] == npiece; l+=13);
		...
		// if (is_attacked (bking_loc, 1)) 
		// display_board(stdout, 1);//return TRUE_C;
	}
}
				 ...
				 int Pep)
{
	if ((Variant != Suicide) && (Pcaptured == wking || Pcaptured == bking))
    {
		...
		if (Pcaptured != npiece) fcaptures = TRUE_C; 
...
void push_king (int target) {
	...
	/* 'normal' capture moves by the king: */
		...
		add_capture(target, board[target], 0, FALSE_C);
		return;
	}
...
void push_pawn (int target, bool_c is_ep) {
	...
	if (is_ep) {
		if (board[gfrom] == wpawn) {
			add_capture(target, bpawn, 0, TRUE_C);
			return;
		}
		else {
			add_capture(target, wpawn, 0, TRUE_C);
			return;
		}
	}
	...
	if (board[gfrom] == wpawn && rank(gfrom) == 7) {
		add_capture(target, captured_piece, wqueen, FALSE_C);
		add_capture(target, captured_piece, wrook, FALSE_C);
		add_capture(target, captured_piece, wbishop, FALSE_C);
		add_capture(target, captured_piece, wknight, FALSE_C);
		if (Variant == Suicide)
			add_capture(target, captured_piece, wking, FALSE_C);
	...
	/* look for a black promotion move: */
		...
		add_capture(target, captured_piece, bqueen, FALSE_C);
		add_capture(target, captured_piece, brook, FALSE_C);
		add_capture(target, captured_piece, bbishop, FALSE_C);
		add_capture(target, captured_piece, bknight, FALSE_C);
		if (Variant == Suicide)
			add_capture(target, captured_piece, bking, FALSE_C);
		/* we've finished generating all the promotions: */
		return;
	}
	...
	/* otherwise, we have a normal pawn move: */
		...
		add_capture(target, captured_piece, 0, FALSE_C);
		return;
	}
