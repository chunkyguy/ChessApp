...
int32_t qsearch (int alpha, int beta, int depth) {
	...
	bool_c legal_move, no_moves = TRUE_C;
...
bool_c remove_one (int *marker, int32_t move_ordering[], int num_moves) {
	
	/* a function to give pick the top move order, one at a time on each call.
     Will return TRUE_C while there are still moves left, FALSE_C after all moves
     have been used */
...
int32_t search (int alpha, int beta, int depth, int is_null) {
	...
	if (num_moves > 0)
    {
		...
		if (legalmoves <= 1 && (Variant != Suicide) && cfg_onerep) threat = TRUE_C;
    }
	else
    {
...
move_s search_root (int originalalpha, int originalbeta, int depth) {
	...
	bool_c no_moves, legal_move, first;
	int alpha, beta;
	move_s kswap;
	move_s oldbest;
	int oldbestnum;
...
restart:
	...
	if (Variant == Losers) captures = TRUE_C;
	else captures = FALSE_C;
	gen(&moves[0]);
	num_moves = numb_moves;
