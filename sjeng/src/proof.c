...
void std_pn_eval (node_t * this)
{
		...
		|| (!white_to_move && is_attacked (bking_loc, BLACK)))
    {
		...
		for (i = 0; i < num_moves; i++)
		{
			...
			if (check_legal (&moves[0], i, TRUE_C))
			{
				mate = FALSE_C;
				unmake (&moves[0], i);
				break;
...
void set_proof_and_disproof_numbers (node_t * node)
{
	...
	else if (node->evaluated)
    {
		...
		else if (node->value == FALSE_C)
		{
			node->proof = +PN_INF;
			node->disproof = 0;
		}
		else if (node->value == TRUE_C)
		{
			node->proof = 0;
...
void develop_node (node_t * node)
{
	...
	for (i = 0; i < num_moves; i++)
    {
		...
		if (check_legal (&moves[0], i, ic))
		{
			...
			newnode->parent = node;
			newnode->evaluated = FALSE_C;
			newnode->expanded = FALSE_C;
	...
	if (node->num_children == 0)
    {
		node->expanded = FALSE_C;
		node->evaluated = TRUE_C;
		if (Variant != Suicide && Variant != Losers)
		{
...
void proofnumberscan (void)
{
	...
	if (root->value == TRUE_C || root->value == FALSE_C)
	{
		Xfree();
		free(membuff);
		free(root);
